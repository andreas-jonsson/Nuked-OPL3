 ...
 * Copyright (C) 2025 Andreas 'phix' Jonsson
 *
...
envelope_sinfunc :: proc(phase, envelope: u16) -> i16
envelope_genfunc :: proc(slott: ^opl3_slot)

OPL3_EnvelopeCalcExp :: proc(level: u32) -> i16
{
    if level > 0x1fff {
        level = 0x1fff
    }
...
OPL3_EnvelopeCalcSin0 :: proc(phase, envelope: u16) -> i16
{
    out, neg: u16
    phase &= 0x3ff
    if bool(phase & 0x200) {
...
OPL3_EnvelopeCalcSin1 :: proc(phase, envelope: u16) -> i16
{
    out: u16
    phase &= 0x3ff
    if bool(phase & 0x200) {
...
OPL3_EnvelopeCalcSin2 :: proc(phase, envelope: u16) -> i16
{
    out: u16
    phase &= 0x3ff
    if bool(phase & 0x100) {
...
OPL3_EnvelopeCalcSin3 :: proc(phase, envelope: u16) -> i16
{
    uint16_t out = 0;
    phase &= 0x3ff;
    if bool(phase & 0x100) {
...
OPL3_EnvelopeCalcSin4 :: proc(phase, envelope: u16) -> i16
{
    out, neg: u16
    phase &= 0x3ff
    if bool(phase & 0x300) == 0x100 {
        neg = 0xffff
    }
    if bool(phase & 0x200) {
        out = 0x1000
    }
    else if bool(phase & 0x80) {
...
OPL3_EnvelopeCalcSin5 :: proc(phase, envelope: u16) -> i16
{
    out: u16
    phase &= 0x3ff
    if bool(phase & 0x200) {
...
OPL3_EnvelopeCalcSin6 :: proc(phase, envelope: u16) -> i16
{
    neg: u16
    phase &= 0x3ff
    if bool(phase & 0x200) {
...
OPL3_EnvelopeCalcSin7 :: proc(phase, envelope: u16) -> i16
{
    out, neg: u16
    phase &= 0x3ff
    if bool(phase & 0x200) {
...
envelope_sin := [8]envelope_sinfunc{
...
OPL3_EnvelopeUpdateKSL :: proc(slot: ^opl3_slot)
{
    ksl: i16 = (kslrom[slot.channel.f_num >> 6] << 2)
...
OPL3_EnvelopeCalc(slot: ^opl3_slot)
{
    nonzero, rate, rate_hi, rate_lo, reg_rate, ks, eg_shift, shift: u8
    eg_rout: u16
    eg_inc: i16
    eg_off, reset: u8
